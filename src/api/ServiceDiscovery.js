...
const AWS_S3_SERVICE_JSON = 'https://s3.us-east-2.amazonaws.com/ndau-json/services.json';
...
const getNodeAddress = async (type) => {
  const node = type === RECOVERY ? await getRecoveryServiceNodeURL() : await getBlockchainServiceNodeURL();
...
const getBlockchainServiceNodeURL = async () => {
  ...
  if (blockchainCache.nodes?.length > 0) {
    ...
    return blockchainCache.nodes[Math.floor(Math.random() * blockchainCache.nodes.length)];
  } else {
...
const getRecoveryServiceNodeURL = async () => {
  ...
  try {
    if (moment().diff(recoveryCache.lastChecked) > CACHE_TTL) {
      const response = await APICommunicationHelper.get(AWS_S3_SERVICE_JSON);
      recoveryCache.nodes = await _parseServicesForNodes(response, RECOVERY);
      recoveryCache.lastChecked = moment();
    }

    // return a random service for use
    return recoveryCache.nodes[Math.floor(Math.random() * recoveryCache.nodes.length)];
  } catch (error) {
    LogStore.log(error);
    throw new ServiceDiscoveryError();
  }
...
const _parseServicesForNodes = async (serviceDiscovery, type) => {
  ...
  if (type === RECOVERY) {
    for (const node of Object.values(serviceDiscovery.recovery[environment].nodes)) {
      nodes.push(node.api);
    }
  } else {
    for (const node of Object.values(serviceDiscovery.networks[environment].nodes)) {
