...
const NDAUDetail = (props) => {
  ...
  const fetchAccount = () => {
    ...
    getNdauAccountDetail(item.address).then((res) => {
      ...
      if (addressData.incomingRewardsFrom?.length && !addressData.incomingRewardsFromNickname) {
        addressData.incomingRewardsFromNickname = UserStore.getAccountDetail(
      ...
      if (addressData.rewardsTarget && !addressData.rewardsTargetNickname) {
        addressData.rewardsTargetNickname = UserStore.getAccountDetail(
      ...
      AccountAPI.getLockRates(UserStore.getAccountDetail(item.address)).then((lockData) => {
        ...
        setAccountInfo((prev) => {
          ...
          const lockBonusEAI = DataFormatHelper.lockBonusEAI(
            DateHelper.getDaysFromISODate(addressData.lock ? addressData.lock.noticePeriod : 0)
          );
          const baseEAI = eaiValueForDisplay - lockBonusEAI;
          let spendableNdau = 0;
          if (!isAccountLocked)
            spendableNdau = AccountAPIHelper.spendableNdau(addressData, true, AppConfig.NDAU_DETAIL_PRECISION);
  ...
  const launchBuyNdauInBrowser = async () => {
    ...
    if (supported) {
      ...
      Alert.alert('Error', `Don't know how to open this URL: ${url}`, [{ text: 'OK', onPress: () => {} }], {
        cancelable: false,
      });
    }
  };
  ...
  const handleNotify = () => {
    setLoading('Starting...');
    notifyForNDAU(UserStore.getAccountDetail(item.address))
      .then((res) => {
        setLoading('');
        setTimeout(() => {
          fetchAccount();
  ...
  const confirm = (onYes) => {
    Alert.alert('Delete account', 'Are you sure that you want to delete the account?', [
    ...
    <ScreenContainer headerTitle={item.name} headerRight={canRecieve && <CopyAddressButton onPress={copyAddress} />}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.headerContainer}>
          ...
          <CustomText titilium body style={{ marginTop: 20, marginBottom: 4 }}>
            {'Balance'}
          </CustomText>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20 }}>
            <CustomText semiBold h4>{`${accountInfo.spendableNdau || item.totalFunds || '0.00'}`}</CustomText>
          ...
          <View style={styles.buttonContainer}>
            <View style={styles.row}>
              ...
              <IconButton
                disabled={accountInfo.isLocked || disableButton}
                label="Send"
                icon={<Send />}
                onPress={() => customModalRef.current(true)}
              />
              <IconButton
                disabled={!canRecieve}
                label="Receive"
                icon={<Receive />}
                onPress={() =>
                  props.navigation.navigate(ScreenNames.Receive, { address: item.address, tokenName: item.tokenName })
                }
              />
            </View>
            <View style={styles.row}>
              ...
              <IconButton
                ...
                label="Lock"
                icon={<Lock />}
                onPress={() => props.navigation.navigate(ScreenNames.LockPeriod, { item })}
              />
            </View>
          </View>
          <View style={styles.infoContainer}>
            ...
            <View style={styles.row}>
              ...
              <CustomText style={styles.margin} titilium>
                {accountInfo.isLocked ? 'Locked' : 'UnLocked'}
              </CustomText>
            </View>
            {accountInfo.isLocked ? (
              <>
                ...
                <View style={styles.row}>
                  <Clock />
                  <CustomText style={styles.margin} titilium>
                    Will unlock on {accountInfo.unlocksOn}
                  </CustomText>
                </View>
              </>
            ) : null}
            ...
            <View style={styles.row}>
              <DollarSign />
              <CustomText style={styles.margin} titilium>
                {accountInfo.spendableNdau}
              </CustomText>
              <CustomText style={styles.margin} color={themeColors.success} titilium>
                Spendable
              </CustomText>
            </View>
            <View style={[styles.row, { marginTop: 20, justifyContent: 'space-between' }]}>
              <CustomText
                style={styles.margin}
                titilium
              >{`${accountInfo.eaiValueForDisplay}% Annualized Incentive (EAI)`}</CustomText>
              <Button
                caption
                rightIcon={<EAI />}
                ...
                onPress={() =>
                  props.navigation.navigate(ScreenNames.EAIDestination, { item, onlySetDestination: true })
                }
              />
            </View>
            <View style={styles.separator} />
      ...
      <CustomModal bridge={customModalRef}>
        <View style={styles.innerModalContainer}>
          <Image style={styles.icon} source={item.image} />
          <CustomText titiliumSemiBold h5 style={{ marginVertical: 6 }}>
            ndau transfer fee
          </CustomText>
          <View style={styles.separator} />
          <CustomText titilium style={styles.textPara}>
            Transaction are subject to a small fee that supports the operations of the ndau network
          </CustomText>
          <View style={styles.transferFeeContainer}>
            <CustomText titiliumSemiBold style={styles.textPara}>
              Transfer Fee
            </CustomText>
            <CustomText titiliumSemiBold style={styles.textPara}>{`(${AppConstants.TRANSACTION_FEE} ndau)`}</CustomText>
          </View>
          <TouchableOpacity onPress={openLink}>
            <CustomText titilium caption style={[{ marginTop: 0, marginBottom: 10 }]}>
              Read more about fees
            </CustomText>
          </TouchableOpacity>
        </View>
        <Button label={'I Understand'} onPress={navigateToSend} />
        <Button
          textOnly
          label={'Cancel'}
      ...
      {accountInfo.isLocked && accountInfo.unlocksOn === null && (
        <Button onPress={handleNotify} label={'Start Count down timer'} buttonContainerStyle={{ marginBottom: 10 }} />
      )}
      <Button label={'View Transaction'} onPress={navigateToTransaction} />
      <Button
