...
const Dashboard = ({ navigation }) => {
  ...
  const makeToken = (type, { totalFunds, usdAmount, accounts }) => {
    const tokens = {
      0: {
        shortName: tokenShortName.NDAU,
        name: 'ndau',
        network: 'ndau',
        totalFunds: '0',
        usdAmount: '0',
        image: images.nDau,
        accounts: getNDauAccounts().length,
      },
      1: {
        shortName: tokenShortName.NPAY,
        name: 'NPAY',
        network: 'zkSync Era',
        totalFunds: '0',
        usdAmount: '0',
      ...
      2: {
        shortName: tokenShortName.ETHERERUM,
        name: 'ETHEREUM',
        network: 'ethereum',
        totalFunds: '0',
        usdAmount: '0',
      ...
      4: {
        shortName: tokenShortName.ZK_ETH,
        name: 'ETHEREUM',
        network: 'zkSync Era',
        totalFunds: '0',
        usdAmount: '0',
      ...
      6: {
        shortName: tokenShortName.MATIC,
        name: 'Matic',
        network: 'Polygon',
        totalFunds: '0',
        usdAmount: '0',
  ...
  const loadBalances = async () => {
    Promise.allSettled([
      ...
      .then((results) => {
        ...
        const totalNdausOnAllAccounts = DataFormatHelper.getNdauFromNapu(
          Object.keys(ndauAccounts)
            .map((key) => ndauAccounts[key])
            .reduce((pv, cv) => (pv += parseFloat(cv.balance)), 0) || 0
        );
        ...
        setTokens([
          makeToken(0, {
            totalFunds: totalNdausOnAllAccounts,
            usdAmount: currentPriceOfNdauInUsd,
            accounts: getNDauAccounts().length,
          }),
  ...
  const refreshData = () => {
    ...
    if (getActiveWallet().type) {
      ...
      setTokens([makeToken(0, { totalFunds: 'l', accounts: getNDauAccounts().length, address: '', usdAmount: 'l' })]);
  ...
  useEffect(() => {
    const availableAllUSDAmount =
      tokens.reduce((prevValue, initial) => (prevValue += parseFloat(initial.usdAmount)), 0) || 0;
    setTotalBalance(parseFloat(availableAllUSDAmount).toFixed(2));
  }, [tokens]);
      ...
      if (selected == 0) return <Token {...item} index={index} onPress={() => handleNavigation(item)} />;
        ...
        return (
          <NFT
            ...
            onPress={() => navigation.navigate(ScreenNames.NFTList, { item })}
          />
        );
    },
    [tokens, nfts]
          ...
          <View style={styles.row}>
            <View style={[styles.buttonContainer]}>
              <Button
                ...
                buttonContainerStyle={[selected === 1 && styles.unSelect]}
                buttonTextColor={selected === 1 ? themeColors.black : themeColors.font}
              />
            </View>
            <View style={[styles.buttonContainer, { marginHorizontal: 10 }]}>
              <Button
                ...
                buttonContainerStyle={[selected === 0 && styles.unSelect]}
                buttonTextColor={selected === 0 ? themeColors.black : themeColors.white}
              />
            </View>
            {/* <View style={[styles.buttonContainer]}>
            ...
            ListEmptyComponent={
              nftLoading && nfts.length === 0 ? (
                ...
                <View style={styles.emptyNFT}>
                  <CustomText color={themeColors.black300} titiliumBold>
                    No NFT found in this account
                  </CustomText>
                </View>
              )
            }
