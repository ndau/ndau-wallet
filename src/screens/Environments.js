...
const Environments = () => {
  ...
  return (
    <ScreenContainer changeHandler={env}>
      <Spacer height={14} />
      <CustomText titilium style={styles.margin}>
        Select which node environments would you like to use.
      </CustomText>
      <Spacer height={20} />
      <View style={styles.container}>
        <FlatList
          ...
          renderItem={({ item, index }) => {
            return (
              <TouchableOpacity
                disabled={env[index].selected && item.id === env[index].id}
                activeOpacity={0.8}
                onPress={() => handleEnv(item)}
              >
                <View style={[styles.item, item.selected && styles.selected]}>
                  <CustomText titilium body style={[{ flex: 1 }]}>
                    {item.name}
                  </CustomText>
                  {item.selected && <Check />}
                </View>
