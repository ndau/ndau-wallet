...
const Transactions = (props) => {
  ...
  const getERCTransaction = () => {
    ...
    getERCTransactionHistory(paramItem.address)
      .then((res) => {
        setLoading('');
        const dates = {};
  ...
  const getTransactionsFoNDAU = () => {
    setLoading('Getting transactions');
    getTransactions(paramItem.address)
      .then((res) => {
        setLoading('');
      ...
      <TouchableOpacity activeOpacity={0.8} onPress={() => handleNavigation(item)}>
        <View style={styles.ndauContainer}>
          <View>
            ...
            <CustomText titilium color={themeColors.black100}>
              {item.TxHash}
            </CustomText>
          </View>
        ...
        <View style={styles.ndauContainer}>
          <View>
            <View>
              ...
              <CustomText titilium color={themeColors.black100}>
                {ndauUtils.truncateAddress(item.from)}
              </CustomText>
            </View>
            <View>
              <CustomText titiliumSemiBold>to: </CustomText>
              <CustomText titilium color={themeColors.black100}>
                {ndauUtils.truncateAddress(item.to)}
              </CustomText>
            </View>
          </View>
          <CustomText titiliumSemiBold>
            {ethers.utils.formatEther(item.value._hex).substring(0, 8)} {paramItem.shortName}
          </CustomText>
        </View>
      </TouchableOpacity>
    );
  ...
  const handleNavigation = (item) => {
    props.navigation.navigate(ScreenNames.TransactionDetail, {
      item: { ...item, isErc: !!paramItem.shortName },
      accountAddress: paramItem?.address,
    });
  };
    ...
    <ScreenContainer headerTitle={item.addressData?.nickname}>
      ...
      <SectionList
        ...
        renderSectionHeader={({ section: { title } }) => (
          <CustomText titiliumSemiBold body style={styles.headerTitle}>
            {title}
          </CustomText>
        )}
        renderItem={({ item }) => {
