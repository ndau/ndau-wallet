...
const createNewAccount = async (wallet, numberOfAccounts = 1) => {
  ...
  const accountCreationKey = wallet.keys[wallet.accountCreationKeyHash].privateKey;
  const pathIndexIncrementor = DataFormatHelper.getNextPathIndex(wallet, KeyPathHelper.accountCreationKeyPath());
