...
const populateWalletWithAddressData = async (wallet) => {
  ...
  const addressDataFromAPI = await AccountAPI.getAddressData(Object.keys(wallet.accounts));
  ...
  for (const walletAccountKey of walletAccountKeys) {
    ...
    if (account.addressData.rewardsTarget) {
      account.addressData.rewardsTargetNickname = addressNicknameMap.get(account.addressData.rewardsTarget);
    }

    if (account.addressData.incomingRewardsFrom) {
      for (const incomingReward of account.addressData.incomingRewardsFrom) {
        account.addressData.incomingRewardsFromNickname = addressNicknameMap.get(incomingReward) + ' ';
      }
    }
...
const nicknameAccount = (account) => {
  const accountNickname = `Account ${account.address.slice(account.address.length - 4, account.address.length)}`;
  ...
  account.addressData.nickname = account.addressData.nickname.replace(/Account \d+\b/, accountNickname);
};
...
const sendSetValidationTransactionIfNeeded = async (wallet, account) => {
  try {
    if (account.addressData.balance > 0 && !account.addressData.validationKeys) {
      LogStore.log(`Sending SetValidation transaction for ${account.addressData.nickname}`);
    ...
    if (account.addressData.balance) {
      LogStore.log(`Issue encountered performing SetValidation: ${JSON.stringify(error)}`);
    }
  }
};
...
const sendDelegateTransactionIfNeeded = async (wallet, account) => {
  try {
    if (
      ...
      LogStore.log(`Sending Delegate transaction for ${account.addressData.nickname}`);
      Object.assign(DelegateTransaction.prototype, Transaction);
      const delegateTransaction = new DelegateTransaction(wallet, account, NodeAddressHelper.getNodeAddress());
      await delegateTransaction.createSignPrevalidateSubmit();
    }
  } catch (error) {
    LogStore.log(`Issue encountered perfroming Delegate: ${JSON.stringify(error)}`);
  }
};
...
const addPrivateValidationKeyIfNotPresent = async (wallet, account) => {
  if (
    ...
    account.addressData.validationScript === AppConfig.GENESIS_USER_VALIDATION_SCRIPT
  ) {
    // Only create the key if we haven't already created it. If we have,
    // then the SetValidation did go through yet...so try it again
    if (!account.validationKeys || (account.validationKeys && account.validationKeys.length === 0)) {
      await ValidationKeyMaster.addValidationKey(wallet, account);
    }

    Object.assign(SetValidationTransaction.prototype, Transaction);
    const setValidationTransaction = new SetValidationTransaction(wallet, account, APIAddressHelper.RECOVERY);
    await setValidationTransaction.createSignPrevalidateSubmit();
  } else {
    await ValidationKeyMaster.recoveryValidationKey(wallet, account, account.addressData.validationKeys);
  }
};

const getEaiValueForDisplay = (account) => {
  return account && account.eaiValueForDisplay
    ? Math.round((account.eaiValueForDisplay / AppConstants.RATE_DENOMINATOR) * 100)
    : 0;
};
...
const isAccountLocked = (account) => {
  ...
  return account.lock && (account.lock.unlocksOn === null || moment(lockedUntil).isAfter(moment.utc().format()));
};
...
const remainingBalanceNdau = (account, amount, addCommas = true, precision) => {
  ...
  return DataFormatHelper.getNdauFromNapu(napuAccountBalance - napuAmount, precision, addCommas);
};
...
const spendableNdau = (addressData, addCommas = true, precision) => {
  return DataFormatHelper.getNdauFromNapu(DataFormatHelper.spendableNapu(addressData, addCommas, precision), precision);
};
...
const accountTotalNdauAmount = (accounts, withCommas = true) => {
  ...
  return withCommas
    ? DataFormatHelper.getNdauFromNapu(totalNapu, AppConfig.NDAU_DETAIL_PRECISION, true)
...
const totalSpendableNdau = (accounts, totalNdau, withCommas = true) => {
  ...
  return withCommas
    ? DataFormatHelper.getNdauFromNapu(totalNapu, AppConfig.NDAU_SUMMARY_PRECISION, true)
    : DataFormatHelper.getNdauFromNapu(totalNapu);
};
 ...
 */
const getTotalNdauForSend = (amountNdau, transactionFeeNapu, sibFeeNapu, addCommas = true) => {
  const amountNapu = DataFormatHelper.getNapuFromNdau(amountNdau, AppConfig.NDAU_DETAIL_PRECISION);
  const totalNapu = amountNapu + transactionFeeNapu + sibFeeNapu;
  return DataFormatHelper.getNdauFromNapu(totalNapu, AppConfig.NDAU_DETAIL_PRECISION, addCommas);
};
...
const currentPrice = (marketPrice, totalNdau) => {
  ...
  const currentPrice = marketPrice
    ? '$' + DataFormatHelper.formatUSDollarValue(parseFloat(totalNdau * marketPrice), 2)
    : '$0.00';
