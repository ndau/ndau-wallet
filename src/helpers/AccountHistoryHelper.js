...
const getAccountHistory = async (address) => {
  ...
  if (accountHistory.Items) {
    for (let i = 0; i < accountHistory.Items.length; i++) {
      const txHistory = await TransactionAPI.transactionByHash(accountHistory.Items[i].TxHash);
...
export const getAccount = async (address) => {
  ...
  try {
    ...
    let accountStateEndpointResponse = await axios.get(accountStateEndpoint, HTTP_REQUEST_HEADER);
...
const getTransactionDestination = (item) => {
  ...
  // only one in here
  return item.txDetail.Transactable.dst ? item.txDetail.Transactable.dst[0] : null;
};
...
const getTransactionSource = (item) => {
  ...
  // that this is an array for a reason but there is currently
  // only one in here
  return item.txDetail.Transactable.src ? item.txDetail.Transactable.src[0] : null;
};
...
const getTransactionBalance = (accountHistory) => {
  return DataFormatHelper.getNdauFromNapu(accountHistory.Balance, AppConfig.NDAU_SUMMARY_PRECISION, true);
};
...
const getAccountHistorywithfilter = async (address, fromDate = Date30DaysAgo, toDate = dateToday) => {
  try {
    const dateToday = new Date(Date.now()).toISOString();
    ...
    while (true && MAX_LOOPS) {
      const url = offset ? `${accountHistoryEndpoint}?after=${offset}` : limitedAccountHistoryEndpoint;
